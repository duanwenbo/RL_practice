...
import numpy as np
import wandb
from maze_continuous import Maze

env = Maze()
wandb.init(project="customize_env_vpg_continuous")
...
class Policy_net(nn.Module):
    def __init__(self, input=env.observation_space.shape[0], hidden=64, output=2):
...
def comput_policy_loss(log_probs, rewards, state_values):
    """
    loss = - mean ( sum( log_prob * advantage ) )
    """
    advantages = compute_advantage(rewards, state_values)[0]
    loss = 0
    for log_prob, advantage in zip(log_probs, advantages):
        step_loss = torch.mul(log_prob, advantage)
        loss += step_loss
    policy_loss = - (loss / len(advantages))
    return policy_loss
...
def verify():
    ...
    state = env.reset()
    while True:
        action = choose_action(state)[0]  # only for verification
...
for i in range(episode_num):
    ...
    policy_loss = comput_policy_loss(log_probs, rewards, state_values)
    ...
    # verification
    result, distance = verify()

    # output info
    print("episode:{}  reward:{}  distance:{}".format(i, result, distance))
    wandb.log({"epoch": i, "reward": result, "distance":distance})